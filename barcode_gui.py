...
def generate_pdf(date_inv, pages, start_number, ratio, progress_bar, status_label, root):
    ...
    pdf.save()
    status_label.config(text=f"PDF généré : {pdf_name}")
...
def main_gui():
    ...
    root = tk.Tk()
    root.title("Générateur de Code-Barres A4")

    frame = ttk.Frame(root, padding=20)
    frame.grid()

    # Champs
    ttk.Label(frame, text="Date inversée :").grid(column=0, row=0, sticky="w")
    date_entry = ttk.Entry(frame)
    date_entry.insert(0, default_date_inv)
    date_entry.grid(column=1, row=0)

    ttk.Label(frame, text="Nombre de pages :").grid(column=0, row=1, sticky="w")
    pages_entry = ttk.Entry(frame)
    pages_entry.insert(0, str(pages_default))
    pages_entry.grid(column=1, row=1)

    ttk.Label(frame, text="Numéro de départ :").grid(column=0, row=2, sticky="w")
    ...
    def on_generate():
        try:
            ...
            ratio = ratio_default  # ratio vient du fichier config.conf
    ...
    generate_button = ttk.Button(frame, text="Générer PDF", command=on_generate)
